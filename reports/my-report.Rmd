---
output:
  title: "Whatever" # É bem melhor um nome que tenha significado
  html_notebook:
    toc: yes
    toc_float: yes
  html_document:
...
```{r}
kable(unique(select(eps, series_name)), floating.environment="sidewaystable")
# quando fazemos knit pra gerar o html, essa tabela fica formatada estranha... 
```
...
kable(unique(select(selected_eps, series_name)), floating.environment="sidewaystable")
```
...
```{r}
kable(select(selected_eps, series_name, season, series_ep, user_rating, user_votes), floating.environment="sidewaystable")
```
...
selected_eps %>%
    ...
    summarise(
        ...
        var = var(user_rating),
        dp = sd(user_rating),
        cvar = (sd(user_rating) / mean(user_rating)),
        max = max(user_rating),
        min = min(user_rating)
    )
```
...
> A média esconde muita informação: qual o máximo? mínimo? há concentração em alguma faixa de valores? Tudo bem usar a média como parte do raciocínio, mas não deixe de usar visualizações mostrando todos os pontos para fazer uma comparação mais rica e mais válida. 
...
> Vale o mesmo comentário de acima.
...
> Mesmo comentário de acima. Usa visualizações. Mostra todos os pontos. Aí você pode ter mais confiança no sumário. 
